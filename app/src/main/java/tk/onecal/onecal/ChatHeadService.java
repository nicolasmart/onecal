...
public class ChatHeadService extends Service {			
	...
	public void onCreate() {
		super.onCreate();
		Log.d(LogTag, "ChatHeadService.onCreate()");
			...
			public boolean onTouch(View v, MotionEvent event) {
				...
				switch (event.getAction()) {
					...
					case MotionEvent.ACTION_UP:
						...
						break;
					default:
						Log.d(LogTag, "chatheadView.setOnTouchListener  -> event.getAction() : default");
						break;
				}
				return true;
			}
	...
	private void resetPosition(int x_cord_now) {
	 ...
	 private double bounceValue(long step, long scale){
	        double value = 0.5 * scale * java.lang.Math.exp(-0.055 * step) * java.lang.Math.cos(0.08 * step);
	        return value;
	    }
	...
	private void showMsg(String sMsg){
		if(txtView != null && chatheadView != null ){
			...
			if(isLeft){								
				param_txt.x = param_chathead.x + chatheadImg.getWidth() + 10;
				param_txt.y = param_chathead.y;
				
				txt_linearlayout.setGravity(Gravity.LEFT | Gravity.CENTER_VERTICAL);
			}else{				
				param_txt.x = param_chathead.x - szWindow.x / 2 - 10;
				param_txt.y = param_chathead.y;
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		Log.d(LogTag, "ChatHeadService.onStartCommand() -> startId=" + startId);
	...
	public IBinder onBind(Intent intent) {
		Log.d(LogTag, "ChatHeadService.onBind()");
		return null;
	}
